...
PROXY_INSTANCE_PROFILE_NAME = os.environ['PROXY_INSTANCE_PROFILE_NAME']
PROXY_DOMAIN                = os.environ['PROXY_DOMAIN']

# NACL rules to be added temporarily to allow maintenance connectivity.
...
def lambda_handler(event, context):
    ...
    valid_modes = ('os-update', 'cert-renew')
    ...
    try:
        results['security_group_add'] = _add_maintenance_sg()
    ...
    try:
        results['iam_instance_profile_add'] = _assign_iam_instance_profile()
    ...
    if mode == 'os-update':
        ...
        try:
            results['apt_maintenance'] = _run_os_update()
    ...
    try:
        results['instance_stop'] = _stop_instance()
    ...
    try:
        results['iam_instance_profile_remove'] = _disassociate_iam_instance_profile()
    ...
    try:
        results['security_group_remove'] = _remove_maintenance_sg()
...
def _assign_iam_instance_profile():
    """Associate PROXY_INSTANCE_PROFILE_NAME with the Proxy instance.
    If an instance profile is already associated, replace it; otherwise create
    a fresh association."""
    logger.info(f"  Checking current IAM instance profile on {PROXY_INSTANCE_ID}")
    iam_profile = {'Name': PROXY_INSTANCE_PROFILE_NAME}

    assoc_response = ec2.describe_iam_instance_profile_associations(
        Filters=[{'Name': 'instance-id', 'Values': [PROXY_INSTANCE_ID]}]
    )
    ...
    if active:
        ...
        if current_name == PROXY_INSTANCE_PROFILE_NAME:
            logger.info(f"  [SKIP] Profile '{PROXY_INSTANCE_PROFILE_NAME}' already associated with {PROXY_INSTANCE_ID}")
        ...
        logger.info(f"  [REPLACE] Replaced existing profile with '{PROXY_INSTANCE_PROFILE_NAME}' on {PROXY_INSTANCE_ID}")
        return {'action': 'replaced', 'instance_profile_name': PROXY_INSTANCE_PROFILE_NAME}

    ec2.associate_iam_instance_profile(
        IamInstanceProfile=iam_profile,
        InstanceId=PROXY_INSTANCE_ID
    )
    logger.info(f"  [ADD ] Associated profile '{PROXY_INSTANCE_PROFILE_NAME}' with {PROXY_INSTANCE_ID}")
    return {'action': 'associated', 'instance_profile_name': PROXY_INSTANCE_PROFILE_NAME}
...
def _add_maintenance_sg():
    """Add MAINTENANCE_SG_ID to the Proxy instance without removing existing groups."""
    logger.info(f"  Checking security groups on {PROXY_INSTANCE_ID}")
    response = ec2.describe_instances(InstanceIds=[PROXY_INSTANCE_ID])
    ...
    logger.info(f"  [ADD ] Maintenance SG {MAINTENANCE_SG_ID} added to {PROXY_INSTANCE_ID}")
    return {'action': 'added', 'security_group_id': MAINTENANCE_SG_ID}


# ---------------------------------------------------------------------------
# APT maintenance helpers (SSM Run Command)
# NOTE: Lambda timeout must be configured to at least 900 seconds (15 minutes)
# ---------------------------------------------------------------------------

def _ensure_instance_running():
    """Start the Proxy instance if not already running and wait until it is."""
    logger.info(f"  Querying state of instance {PROXY_INSTANCE_ID}")
    response = ec2.describe_instances(InstanceIds=[PROXY_INSTANCE_ID])
    ...
    if state in ('terminated', 'shutting-down'):
        raise RuntimeError(f"Instance {PROXY_INSTANCE_ID} is in unrecoverable state: {state}")

    if state == 'stopping':
        logger.info(f"  [WAIT] Instance is stopping; waiting for 'stopped' before starting...")
        for _ in range(12):  # up to ~2 minutes
            time.sleep(10)
            resp = ec2.describe_instances(InstanceIds=[PROXY_INSTANCE_ID])
            if resp['Reservations'][0]['Instances'][0]['State']['Name'] == 'stopped':
                break

    if state in ('stopped', 'stopping'):
        ec2.start_instances(InstanceIds=[PROXY_INSTANCE_ID])
        logger.info(f"  [START] Start signal sent to instance {PROXY_INSTANCE_ID}")
    ...
    raise RuntimeError(f"Instance {PROXY_INSTANCE_ID} did not reach 'running' state within timeout")


def _wait_for_ssm_agent(max_polls=24):
    """Wait until the SSM agent on the Proxy instance reports as Online."""
    logger.info(f"  Polling SSM agent on {PROXY_INSTANCE_ID} (max {max_polls} polls x 10s = ~{max_polls * 10}s)")
    for attempt in range(max_polls):  # up to ~4 minutes
        time.sleep(10)
        response = ssm.describe_instance_information(
            Filters=[{'Key': 'InstanceIds', 'Values': [PROXY_INSTANCE_ID]}]
        )
    ...
    raise RuntimeError(f"SSM agent on {PROXY_INSTANCE_ID} did not come online within timeout")


def _run_ssm_command(commands, label, timeout_seconds=600):
    """Send shell commands via SSM Run Command and poll until complete."""
...
def _run_os_update():
    ...
    logger.info("--- [CMD 1/4] Ensuring Proxy instance is running ---")
    instance_result = _ensure_instance_running()

    logger.info("--- [CMD 2/4] Waiting for SSM agent to come online ---")
    ssm_agent_result = _wait_for_ssm_agent()
...
def _run_cert_renew():
    """Ensure instance is running, wait for SSM agent, then renew the certificate and restart haproxy."""
...
def _stop_instance():
    """Stop the Proxy instance (fire-and-forget; does not wait for stopped state)."""
    logger.info(f"  Querying state of instance {PROXY_INSTANCE_ID}")
    response = ec2.describe_instances(InstanceIds=[PROXY_INSTANCE_ID])
    state = response['Reservations'][0]['Instances'][0]['State']['Name']
    logger.info(f"  Current state: [{state.upper()}]")
    ...
    ec2.stop_instances(InstanceIds=[PROXY_INSTANCE_ID])
    logger.info(f"  [STOP] Stop signal sent to instance {PROXY_INSTANCE_ID}")
    return {'action': 'stopping'}


def _disassociate_iam_instance_profile():
    """Remove any active IAM instance profile association from the Proxy instance."""
    logger.info(f"  Checking IAM instance profile associations on {PROXY_INSTANCE_ID}")
    assoc_response = ec2.describe_iam_instance_profile_associations(
        Filters=[{'Name': 'instance-id', 'Values': [PROXY_INSTANCE_ID]}]
    )
    active = [
    ...
    if not active:
        logger.info(f"  [SKIP] No active instance profile found on {PROXY_INSTANCE_ID}")
    ...
    logger.info(f"  [REMOVE] Disassociated profile '{profile_name}' (id={association_id}) from {PROXY_INSTANCE_ID}")
    return {'action': 'disassociated', 'association_id': association_id}


def _remove_maintenance_sg():
    """Remove MAINTENANCE_SG_ID from the Proxy instance's security groups."""
    logger.info(f"  Checking security groups on {PROXY_INSTANCE_ID}")
    response = ec2.describe_instances(InstanceIds=[PROXY_INSTANCE_ID])
    instance = response['Reservations'][0]['Instances'][0]
    current_sg_ids = [sg['GroupId'] for sg in instance['SecurityGroups']]
    ...
    if MAINTENANCE_SG_ID not in current_sg_ids:
        logger.info(f"  [SKIP] Maintenance SG {MAINTENANCE_SG_ID} not found on {PROXY_INSTANCE_ID}")
